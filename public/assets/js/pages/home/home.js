    ...
    $.ajax({
        ...
        success: function (response) {
            // console.log("get_corporate_requests =>",response);
            if (response.responseCode == "000") {
                $(".rquest_table tr").remove()
                let data = response.data;
                console.log("get_corporate_requests =>", data.length);
                if(data.length > 0){
        ...
        error: function (xhr, status, error) {
            ...
            // };
                                ...
                                if (response.responseCode == '000') {
                                    ...
                                    if (response.data.length > 0) {
                                        ...
                                        $.each(data, function(index) {
                                            ...
                                            // table.row.add([
                                            //     `<b> ${data[index].interestAccount} </b>`,
                                            //     `<b class="float-right"> ${ formatToCurrency(parseFloat(data[index].dealAmount))} </b>`,
                                            //     `<b> ${data[index].tenure} </b>`,
                                            //     `<b> ${interest_rate} </b>`,
                                            //     `<b> ${rollover_ } </b>`
                    ...
                    function get_accounts(account_data) {
                        // $("#accounts_display_area").DataTable()
